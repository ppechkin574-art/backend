...
## 📚 Phase 7b kk-перевод — качество контента (открыто 22.05.2026)

После раскатки kk-перевода на все 12 предметов (`de4344d`, 2817/2821 = 99.86% coverage) визуальный обход на симе подтвердил **технику** работающей. Но **сам контент** перевода — это data quality issue, не код.

### № 1. Неправильные окончания и смысловые последовательности в части kk-вопросов

**Кто сообщил:** оператор-носитель казахского языка 22.05.2026 (визуальный обход Биология + Химия на симе).

**Что наблюдается:**
- В некоторых вопросах **неправильные окончания** в казахском тексте (видимо AI-перевод не учёл падежи / число / лицо).
- **Смысловые последовательности** местами хромают — порядок слов / связки между предложениями.

**Почему так:** source-файл `/Users/macbookpro/mare-ent-db/data/questions_export_kk.json` (2821 entries) был сгенерирован внешним переводным процессом (AI-aided, не вычитан носителем). Тот же недостаток оператор уже фиксировал в C1 (subscription-benefits): «Локаль казахского — "понятное" качество, заказчик может полировать через UI».

**Что НЕ нужно делать:**
- Не править отдельные строки в БД вручную — десятки тысяч строк, ROI околонулевой.
- Не пытаться чинить через автоматический rewrite (AI-on-AI) — деградирует ещё сильнее.

**Что нужно сделать (требует ресурса носителя):**
- Носитель проходится по source-файлу + правит неудачные переводы. Можно поэтапно — по приоритету subjects (Math + History KZ первые, остальные потом).
- После каждой партии правок — заново сгенерировать seed-файл и прогнать миграцию (паттерн `[[feedback_phase7b_translation_pattern]]`).

**Известные структурно-сломанные qids (kk-фрагменты внутри `r"..."` math-блоков), занулены в `f1a2b3c4d5e7`:** 4252, 4289, 4354, 4361 (Math). При полировке source стоит починить именно их `question_text` в первую очередь — после этого их можно re-import'нуть (миграция идемпотентна).

**Что НЕ относится к этому пункту (отдельные хвосты):**
- LaTeX-рендер `\frac` с длинными формулами — баг `flutter_math_fork`, см. `latex_sanitizer.dart` для текущих обходов.
- RU-надписи на статичных PNG-изображениях (ЭСКИЗ СЦЕНЫ и т.д.) — это контент-файл, не текст.
- Cyrillic-TeX (`\бұрыш`, `\массив`) — кроется sanitizer-map'ом, не data quality.

**Связанные коммиты Phase 7b** (для контекста при правках):
- `17205aa` — kk в create-flow
- `5c7fc75` + `e7d8c9b1a2f3` — variants kk
- `3f80ee2` — multi-block drop
- `f1a2b3c4d5e7` — patch nullified qids + manual qid 4325
- `de4344d` + `f2b3c4d5e6a8` — раскатка на 11 предметов

Pattern для будущих re-import'ов: `[[feedback_phase7b_translation_pattern]]` в Claude memory.

### № 2. Дословный перевод теряет смысл вопроса

**Кто сообщил:** оператор-носитель казахского 22.05.2026 (визуальный обход на симе).

**Что наблюдается:**
- В некоторых вопросах после перевода остаётся буквально «Кім?» («Кто?») без контекста и пояснения, что именно спрашивается.
- Это происходит когда AI-переводчик сократил RU-вопрос-связку (типа «Кто из перечисленных является автором...?») до одного слова, потеряв связку и контекст.

**Почему так:** тот же AI-source-issue что и в №1 — переводчик не сохраняет полноту смысла, особенно когда RU-вопрос имеет сложную конструкцию с вводным предложением.

**Что делать (вместе с №1):** проходить source-файл носителем + добавлять/восстанавливать контекст где AI его съел. Pattern re-import тот же.

### № 3. Ағылшын тілі — переведён ВЕСЬ контент включая английский

**Кто сообщил:** оператор 22.05.2026.

**Что наблюдается:** для предмета «Ағылшын» (английский язык) AI перевёл на казахский **и сам английский контент тоже**. Это ломает суть теста по английскому. Примеры:

| qid | Что в БД сейчас (kk) | Что должно быть |
|---|---|---|
| 599 | «Зат есімнің дұрыс түрі Thank you for your … .» + варианты `[кеңес, кеңес береді, кеңес беру, кеңестер]` | KZ-стем + английские варианты `[advice, advises, advising, advices]` |
| 546 | «Дұрыс пішін….. бүгін өте желді» + `[Бар, Болды, Бар, Бұл]` | KZ-стем + «… very windy today» + `[is, was, are, it is]` |
| 547 | «Дұрыс нұсқа Біз… кейін –ing пішінін ҚОЛДАНМАЙМЫЗ…» + `[Жеп көремін, Мен қарсы емеспін, …]` | KZ-стем + после "after we" + английские глагольные конструкции |

**Решение применённое 22.05.2026 (commit `XXXXXXX`):**
- Бэк-патч-миграция NULL'ит `question_text_kk` / `hint_text_kk` / `variant_text_kk` для всего subject='Ағылшын'.
- Эффект: фронт возвращается к RU fallback'у — а в RU оригинале английский контент сохранён правильно (KZ→RU stems + English content body). На казахском UI пользователь видит вопрос на русском, что менее идеально чем правильный kk-перевод, но **тест по английскому работает корректно**.

**Что нужно для полноценного fix'а (требует ресурса носителя):**
- Перепереводить только **stems и инструкции** на казахский, оставляя английский контент (предложения для grammar exercise, варианты ответов) на английском.
- Сгенерить отдельный seed `english_kk_stems_only.json` + ре-импорт через идемпотентную миграцию.
- Это особенный subject и требует ручной обработки — нельзя auto-translate.

**Тот же риск может быть в других subjects где есть English или иной не-RU контент:**
...
## 🔴 PS.KZ VPS — открытый тех-долг по хостингу (после переезда с Railway)

Backend/admin/keycloak/landing теперь реально крутятся на выделенном VPS
(PS.KZ, `213.155.20.120`, «Server-Aima») — см. `app/CLAUDE.md`, раздел про
миграцию Railway → PS.KZ. Два пункта именно по самому хостингу VPS, не по
коду:

### Автопродление аренды сервера выключено

На странице сервера в консоли ps.kz (`console.ps.kz` → Server-Aima): тариф
Basic-3, аренда оплачена **до 08.08.2026**, «Автопродление» выключено
(кнопка «Включить», не активный тумблер). Если не продлить вручную и не
включить автопродление до этой даты — сервер может быть приостановлен, что
разом уронит весь прод (сайт, API, админку, Keycloak).
**Действие:** зайти в console.ps.kz → Server-Aima → включить автопродление
или явно продлить аренду.

### Снапшоты уровня хостера не проверены

На той же странице сервера есть блок «Автоматическое резервное
копирование → Управление копиями» — снапшоты всего сервера средствами
ps.kz (диск + ОС целиком), отдельно от уже настроенных собственных
ежедневных `pg_dump`-бэкапов баз (`/opt/aima/backup.sh` + cron на сервере,
хранит `aima`+`keycloak` дампы 14 дней). Не проверяли, включена ли эта
опция хостера и что именно она покрывает (возможно, платно отдельно).
**Действие:** зайти в «Управление копиями», понять условия, решить
включать ли — вторая линия защиты на случай проблем не только с БД, но и
с самим диском/ОС.
